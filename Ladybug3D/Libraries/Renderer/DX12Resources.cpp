...
namespace Ladybug3D {
	...
	bool DX12Resources::Initialize(HWND hwnd, UINT width, UINT height)
	{
		try {
			m_width = width;
			m_height = height;
	...
	void DX12Resources::ResizeSwapChainBuffer(UINT width, UINT height)
	{
	...
	void DX12Resources::GetDebugInterface()
	{
	...
	void DX12Resources::GetAdapters(bool useWarp)
	{
	...
	void DX12Resources::CreateDevice(IDXGIAdapter4* adapter)
	{
	...
	void DX12Resources::CreateCommandQueue()
	{
	...
	void DX12Resources::CreateSwapChain(HWND hwnd)
	{
		UINT createFactoryFlags = 0;
#if defined(_DEBUG)
		createFactoryFlags = DXGI_CREATE_FACTORY_DEBUG;
	...
	void DX12Resources::CreateMainRTV()
	{
	...
	_Use_decl_annotations_
		void DX12Resources::GetHardwareAdapter(
	...
	void DX12Resources::ClearMainRTV()
	{
		for (auto& rtv : m_renderTargets) {
			rtv.Reset();
		}
		for (auto& dsTexture : m_DepthStencilTextures) {
			dsTexture->Reset();
		}
	}
}